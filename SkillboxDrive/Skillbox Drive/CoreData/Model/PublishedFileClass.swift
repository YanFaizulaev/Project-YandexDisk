...
class PublishedFileClass {
    
    private lazy var fetchedResultController : NSFetchedResultsController<PublishedFile> = {
        let fetchRequest = PublishedFile.fetchRequest()
        ...
        do {
            ...
            if arrayFetched.isEmpty && !array.isEmpty {
                array.forEach { file in
                  let newFile = PublishedFile.init(entity: NSEntityDescription.entity(forEntityName: "PublishedFile", in: CoreDataManager.shared.context)!, insertInto: CoreDataManager.shared.context)
                ...
                let fetchRequestDelete = PublishedFile.fetchRequest()
                arrayDeleteFromData.forEach { file in
                    ...
                    do {
                        let object = try fetchedResultController.managedObjectContext.fetch(fetchRequestDelete)
                        if !object.isEmpty {
                        object.forEach { file in
    ...
    func connectContext() -> [Items]{
        ...
        let fetch = PublishedFile.fetchRequest()
