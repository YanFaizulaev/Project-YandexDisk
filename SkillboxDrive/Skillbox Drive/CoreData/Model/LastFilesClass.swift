...
class LastFilesClass {
        ...
        do {
            ...
            if arrayFetched.isEmpty && !array.isEmpty {
                array.forEach { file in
                  let newFile = LastFiles.init(entity: NSEntityDescription.entity(forEntityName: "LastFiles", in: CoreDataManager.shared.context)!, insertInto: CoreDataManager.shared.context)
                ...
                let fetchRequestDelete = LastFiles.fetchRequest()
    ...
    func connectContext() -> [Items]{
        ...
        let fetch = LastFiles.fetchRequest()
